...
type (
	homestayModel interface {
		...
		Update(ctx context.Context, session sqlx.Session, data *Homestay) error
...
func (m *defaultHomestayModel) Update(ctx context.Context, session sqlx.Session, data *Homestay) error {
	homestayIdKey := fmt.Sprintf("%s%v", cacheHomestayIdPrefix, data.Id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
